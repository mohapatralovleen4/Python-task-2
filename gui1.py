...
class Mark(tk.Frame): 
	def __init__(self,parent,controller): 
		...
		subject1_button = ttk.Button(self, text ="Physics", 
							command = lambda : self.show_subject(1)) 
		subject1_button.grid(row = 1, column = 1, padx = 10, pady = 10) 

		subject2_button = ttk.Button(self, text ="Applied Mechanics", 
							command = lambda : self.show_subject(2)) 
		subject2_button.grid(row = 2, column = 1, padx = 10, pady = 10) 		

		subject3_button = ttk.Button(self, text ="Chemistry", 
							command = lambda : self.show_subject("3")) 	
		subject3_button.grid(row = 3, column = 1, padx = 10, pady = 10) 
		mark_button = ttk.Button(self, text ="Submit", 
							command = lambda : self.save_mark()) 
		mark_button.grid(row = 4, column = 1, padx = 10, pady = 10) 
	...
	def show_subject(self,subject):	
		...
		if subject == 1 :
			...
			mark_string = "Enter mark for Applied Mechanics"
		else:
			mark_string = "Enter mark for Applied Chemistry"

		mark = simpledialog.askstring("Input", mark_string)
		if subject == 1 :
...
class Grade(tk.Frame): 
	...
	def show_cgpa(self):
		...
		conn = sqlite3.connect("student.db")
		cur = conn.cursor()
		grade_list = cur.execute("SELECT subject_1_mark,subject_2_mark,subject_3_mark FROM student_details WHERE id=?", (app.current_id,))
